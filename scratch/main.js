...
(function() {
  var board, boards, camera, canvasHeight, canvasWidth, clickedPoints, clock, controls, floor, foo, geometry, getBarycentricCoords, hud, hudCamera, hudScene, light, material, pickedObject, pointerLocked, render, renderImage, renderer, scene, t, updateBoards;
  ...
  updateBoards = function() {
    var image, texture, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = boards.length; _i < _len; _i++) {
      board = boards[_i];
      image = renderImage(canvasWidth, canvasHeight, function(ctx) {
        ctx.fillStyle = "#FFFFFF";
        ctx.fillRect(0, 0, canvasWidth, canvasHeight);
        return _.each(clickedPoints, function(point) {
          ctx.fillStyle = "#FF0000";
          return ctx.fillRect(Math.floor(point[0] * canvasWidth), Math.floor(point[1] * canvasHeight), 10, 10);
        });
      });
      texture = new t.Texture(image);
      texture.needsUpdate = true;
      board.material.setValues({
        map: texture
      });
